 ...
 */
typedef struct {
	volatile int32_t Absolute;        /*!< Absolute rotation from beginning, for public use */
	volatile int32_t Diff;            /*!< Rotary difference from last check, for public use */
	TM_RE_Rotate_t Rotation; /*!< Increment, Decrement or nothing, for public use */
	TM_RE_Mode_t Mode;       /*!< Rotary encoder mode selected */
	volatile uint8_t LastA;           /*!< Last status of A pin when checking. Meant for private use */
	volatile int32_t RE_Count;        /*!< Temporary variable to store data between rotation and user check */
